...
def initialize_optimizer_and_scheduler(model, train_loader):
    if OPTIMIZER == "sgd":
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE,eps=1e-6,momentum=0.9)
    else:
...
def train_model(model, train_loader, val_loader, label_names, weights, training_stats):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with tqdm(train_loader, unit="batch", desc=f"Epoch {epoch+1}") as pbar:
            for batch in pbar:
                ...
                for i in range(len(preds)):
                    valid = labels[i] != -100
                    all_train_preds.append([label_names[p] for p in preds[i][valid]])
                    all_train_labels.append([label_names[l] for l in labels[i][valid]])
        ...
        # Store statistics
        training_stats['loss'].append(epoch_loss / len(train_loader))
        training_stats['training_f1_scores'].append(train_f1)
