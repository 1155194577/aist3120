...
def train_model(model, train_loader, val_loader, label_names, weights, training_stats):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Calculate F1 score for the training dataset
        train_report = validate_model(model, train_loader, label_names, device)
        train_f1 = train_report["micro avg"]["f1-score"]
        training_stats['train_f1_scores'].append(train_f1)

        # Validate on the validation dataset
        val_report = validate_model(model, val_loader, label_names, device)
        val_f1 = val_report["micro avg"]["f1-score"]
        recall = val_report["micro avg"]["recall"]
        precision = val_report["micro avg"]["precision"]
...
def plot_results(training_stats):
    ...
    plt.plot(x, training_stats['train_f1_scores'], label='Training F1 Score', color='green')
#     plt.plot(x, training_stats['recalls'], label='Validation Recall')
#     plt.plot(x, training_stats['precisions'], label='Validation Precision')
    plt.plot(x, training_stats['f1_scores'], label='Validation F1-Score',color='red')
